...
import { IpiresiesModel, IIpiresies } from './ipiresies.model';
import { ResourceController } from '../../shared';
import { StatusCodes } from 'http-status-codes';
import { Logger } from '../../shared/utils/logger';

export class IpiresiesController extends ResourceController<IIpiresies>{

    private logger: Logger = new Logger();
    constructor() {
        super(IpiresiesModel);
    }
    /**
    ...
    initializeItems = async (req: Request, res: Response) => {
        ...
        await IpiresiesModel.insertMany(items)
            .then(function (docs) {
                res.json(docs);
            })
            .catch(function (err) {
